...
contract Marketing is OwnerWithdrawable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    function holdTokensFor(address _user, uint256 _amount) external onlyOwner {
        require(_amount > 0, "PreSale: Amount cannot be zero");
        require(_user != address(0), "PreSale: Buyer cannot be zero address");

        IERC20(token).safeTransferFrom(msg.sender, address(this), _amount);
        userAmount[_user] = userAmount[_user].add(_amount);
    }

    function withdrawToken() external {
        uint256 amount = userAmount[msg.sender];
        require(isUnlockingStarted, "Presale: Locking period not over yet");
        require(amount > 0, "Presale: you don't have any tokens to withdraw");
